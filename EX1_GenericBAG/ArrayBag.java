...
public final class ArrayBag<T> implements BagInterface<T> {
 ...
 //   private static Random r = new Random();
    ...
    public T remove() {
        ...
        T result = null;
        Random r = new Random();
        if (!isEmpty() && (numberOfEntries >= 0)) {
	        int remove = r.nextInt(numberOfEntries);
	        result = removeEntry(remove);
        }
	    ...
	    if(aBag.getCurrentSize()==numberOfEntries){
		    for(int i = 0; i < aBag.getCurrentSize(); i++){
		    	for(int j = 0; j < aBag.getCurrentSize(); j++){
			    	if(aBag.bag[i]==bag[j]){
			    		tempBag[j] = null;
			    		j = aBag.getCurrentSize();
			    	}
		    	}
		    }
		    for(int z = 0; z < aBag.getCurrentSize(); z++){
		    	if(tempBag[z]!=null){
		    		result = false;
		    	}
		    }
	...
	public void removeDuplicates() {
	    ...
	    if(length>0){
	    	for(int i = 0; i < this.getCurrentSize(); i++){
	    		for(int j = i+1; j < this.getCurrentSize(); j++){
		    		if(bag[i]==bag[j]){
		    			result = removeEntry(j);
		    			j--;
		    		}
	    		}
	    	}
